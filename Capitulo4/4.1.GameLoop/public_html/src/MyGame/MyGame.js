...
MyGame.prototype.update = function () {
    ...
    var deltaX = -0.01;//Si delta es positivo entonces se mueve a la derecha y si es negativo a la izquierda
    if (whiteXform.getXPos() > 30) // Si su posición es mayor a la esquina derecha inicie en la misma posición
        whiteXform.setPosition(10, 60);
    whiteXform.incXPosBy(deltaX);//Desplacese en el eje horizontal a la tasa delta
    whiteXform.incRotationByDegree(3);//Jire sobre su eje a la siguiente velocidad

    // Step B: pulse the red square
    var redXform = this.mRedSq.getXform();
    if (redXform.getWidth() > 5)
        redXform.setSize(2, 2);
    redXform.incSizeBy(0.05);//Incrementar el tamaño en tasa de 0.05 mientras el ancho sea menor a 5
};